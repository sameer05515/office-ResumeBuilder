...
public class DBConnect {
	...
	private static String DATABASE_NAME = "resumebuilder";
	private static String DB_URL = "jdbc:postgresql://localhost:5432/" + DBConnect.DATABASE_NAME;
	private static String USER_NAME = "postgres";
	private static String PASSWORD = "vinu123";
	...
	public boolean insertPersonalDetails(Personal personal) {
		String SQL = "INSERT INTO personal_details(name, father_name, sex, dob, age, "
				...
				+ "mother_name, nationality, career_objectives, strengths, hobbies, surname, place) " + "VALUES('"
				+ personal.getName() + "', '" + personal.getFatherName() + "','" + personal.getGender() + "','"
				+ personal.getDob() + "','" + personal.getAge() + "','" + personal.getMobileNo() + "','"
				+ personal.getMaritalStatus() + "','" + personal.getAddress() + "','" + personal.getEmailId() + "','"
				+ personal.getLangaugeProf() + "','" + personal.getMotherName() + "','" + personal.getNationality()
				+ "','" + personal.getCareerObjectives() + "','" + personal.getStrengths() + "','"
				+ personal.getHobbies() + "','" + personal.getSurname() + "','" + personal.getPlace() + "')";
	...
	public boolean insertEducationalDetails(Education education) {
		String SQL = "INSERT INTO eductional_details (name, degree, " + "subject, year, board_or_university, "
				+ "obtained_marks, total_marks,percentage) " + "VALUES('" + education.getCollegeName() + "','"
				+ education.getDegree() + "','" + education.getSubject() + "', '" + education.getYear() + "','"
				+ education.getBoard() + "', '" + education.getObtainedMarks() + "', '" + education.getTotalMarks()
	...
	public boolean insertExperienceDetails(Experience experience) {
		String SQL = "INSERT INTO experience (employer, designation, "
				+ "joining_date, leaving_date, work, experience) " + "VALUES('" + experience.getEmployer() + "', '"
				+ experience.getDesignation() + "', '" + experience.getJoiningDate() + "', '"
				+ experience.getLeavingDate() + "', '" + experience.getWorking() + "', '" + experience.getExperience()
				+ "')";
		System.out.println("SQL >> " + SQL);
		connection = getConnection();
		boolean result = Boolean.FALSE;
	...
	public boolean insertParticipation(Participation part) {
		String SQL = "insert into participation(part_name, start_date, " + "end_date, sponsores, note, topic, place)"
				+ " values( '" + part.getParticipation().name() + "', '" + part.getStartDate() + "', '"
				+ part.getEndDate() + "', '" + part.getSponsores() + "', '" + part.getNote() + "','" + part.getTopic()
				+ "','" + part.getPlace() + "')";
		System.out.println("SQL >> " + SQL);
	...
	public boolean insertAchievement(Achievements achievement) {
		String SQL = "insert into achievement(name, title, place, ach_date)" + " values ('" + achievement.getName()
				+ "','" + achievement.getTitle() + "','" + achievement.getPlace() + "','" + achievement.getDate()
				+ "')";
		System.out.println("SQL >> " + SQL);
		boolean result = false;
		connection = getConnection();
	...
	public boolean insertProject(Projects projects) {
		String SQL = "insert into projects(title, guide," + "front_end, back_end, input, output, description)"
				+ " values('" + projects.getTitte() + "','" + projects.getGuide() + "','" + projects.getFrontEnd()
				+ "','" + projects.getBackEnd() + "','" + projects.getInputs() + "','" + projects.getOutputs() + "','"
	...
	public boolean insertReference(References reference) {
		String SQL = "insert into reference(name, designation, affilation, email, contact_no)" + " values('"
				+ reference.getName() + "','" + reference.getDesignation() + "','" + reference.getAffiliation() + "','"
				+ reference.getEmailId() + "','" + reference.getContactNo() + "')";
		System.out.println("SQL >> " + SQL);
		boolean result = false;
		connection = getConnection();
		try {
		...
		try {
			if (connection != null) {
				...
				while (rs.next()) {
					...
					participation.add(new Participation(Participation.getParticipation(rs.getString("part_name")),
							rs.getString("start_date"), rs.getString("end_date"), rs.getString("sponsores"),
							rs.getString("note"), rs.getString("topic"), rs.getString("place")));
